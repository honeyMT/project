...
retry:
	...
	* 注意current是一个宏, 代表当前进程 */
	 ...
	 * there's still timeout left over, we go trying again in search of* more luck.
...
static int ep_scan_ready_list(struct eventpoll *ep,int(*sproc)(struct eventpoll *,struct list_head *, void *),void *priv)
{
	...
	* 要注意哦, rdllist已经被清空了! */
	...
	?/* 在这个回调函数里面处理每个epitem* sproc 就是 ep_send_events_proc, 下面会注释到. */
	error = (*sproc)(ep, &txlist, priv);
	spin_lock_irqsave(&ep->lock, flags);
	?/** During the time we spent inside the "sproc" callback, some
	 * other events might have been queued by the poll callback.
	 * We re-insert them inside the main ready-list here.*/
	/* 现在我们来处理ovflist, 这些epitem都是我们在传递数据给用户空间时监听到了事件. */
...
static int ep_send_events_proc(struct eventpoll *ep, struct list_head *head,void *priv)
{
	...
	* holding "mtx" during this call.*/
	...
	for (eventcnt = 0, uevent = esed->events;
		...
		?/* 然后从链表里面移除 */
		list_del_init(&epi->rdllink);
		?/* 读取events,*注意events我们ep_poll_callback()里面已经取过一次了, 为啥还要再取?
...
static void ep_free(struct eventpoll *ep)
{
	...
	????/* 之所以在关闭epollfd之前不需要调用epoll_ctl移除已经添加的fd,是因为这里已经做了... */
